        ...
        <Card className="glass-strong w-full">
          <CardContent className="p-8">
            <div className="mb-8 flex flex-col items-center text-center">
              <Logo markClassName="h-14 w-14" className="mb-3 flex-col gap-2" showTagline />
              ...
              <div className="space-y-3">
                {ROLES.map(({ role, icon: Icon, labelKey }) => (
                  <button
                    key={role}
                    onClick={() => enter(role)}
                    className="group flex w-full items-center gap-4 rounded-xl border border-white/10 bg-white/[0.03] p-4 text-start transition-all hover:border-violet-400/50 hover:bg-white/[0.07] hover:shadow-glow-sm"
                  >
                    <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 text-white shadow-glow-sm">
